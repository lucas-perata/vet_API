...
namespace API.Controllers
{
    ...
    public class AppointmentController : ControllerBase
    {
        ...
        public AppointmentController(IMapper mapper, AppointmentRepository appointmentRepository, UserManager<AppUser> userManager)
        {
        ...
        [HttpGet("/vet")]
        public async Task<ActionResult<List<AppointmentDto>>> GetAppointmentsForVet(AppointmentDto appointmentDto)
        {
        ...
        [HttpGet("/owner")]
        public async Task<ActionResult<List<AppointmentDto>>> GetAppointmentsForOwner(AppointmentDto appointmentDto)
        {
            var email = User.FindFirstValue(ClaimTypes.Email); 
            var user = await _userManager.FindByEmailAsync(email); 
        ...
        [HttpPost]
        public async Task<ActionResult> RequestAppointment(RequestAppointmentDto requestAppointmentDto)
        {
            var email = User.FindFirstValue(ClaimTypes.Email); 
            var user = await _userManager.FindByEmailAsync(email);
            ...
            if(await _appointmentRepository.Complete() is false) return BadRequest();
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> AcceptOrUpdateAppointment(UpdateAppointmentDto updateAppointmentDto, int id)
        {
            ...
            if(appointment.Date == updateAppointmentDto.Date)
            {
                ...
                if(await _appointmentRepository.Complete()) return Ok("Appointment accepted"); 
                return BadRequest();
            }
            ...
            _appointmentRepository.UpdateAppointment(appointment); 
            if ( await _appointmentRepository.Complete()) return Ok("Date changed"); 
        ...
        public async Task<ActionResult> DeleteAppointment(int id)
        {
            ...
            if(appointment.OwnerId != user.Id || appointment.VetId != user.Id) return BadRequest();
